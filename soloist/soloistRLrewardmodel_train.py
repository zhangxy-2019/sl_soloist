...
class RLJsonDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', max_seq=80, max_turn=1, seperator=' & '):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            self.RLexamples = []
            self.RLtoken_ids = []
            # self.gt_bsresponse_ids = []
            # self.gt_belief_ids = []
            self.gt_belief_length = []
            self.gt_response_length = []
            self.gt_response_ids = []
            # self.labels = []
            self.mc_labels = []
            # self.examples = []
            # self.token_ids = []
            ...
            for id, example in enumerate(examples):
                ...
                belief_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(belief))
                gt_response = example['reply'] + ' '+tokenizer.eos_token
                response_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(gt_response))
                # bs_response_ids = belief_id + response_id
                ...
                # self.gt_bsresponse_ids.append(bs_response_ids)
                # self.labels.append(1)
                self.mc_labels.append(0)
                self.gt_belief_length.append(len(belief_id))
                self.gt_response_length.append(len(response_id))
                self.gt_response_ids.append(response_id)

                if args.replace_systurn:
                    for _ in range(args.num_candidates):
                        ...
                        self.gt_belief_length.append(len(belief_id))
                        self.gt_response_length.append(len(new_response_id))
                        self.gt_response_ids.append(new_response_id)
                    
                if args.repeat_token:
                    for _ in range(args.num_candidates):
                        idx_len = list(range(len(response_id)))
                        sample_idxs = copy.deepcopy(idx_len)
                        # sample_idxs.remove(id)
                        random_idx = random.choice(sample_idxs)
                        new_response_id = copy.deepcopy(response_id)
                        rep_token = new_response_id[random_idx]
                        new_response_id.insert(random_idx, rep_token)
                        new_response_id.insert(random_idx, rep_token)
                        new_response_id.insert(random_idx, rep_token)
    ...
    def __getitem__(self, item):
        return torch.tensor(self.RLexamples[item]), self.RLtoken_ids[item], self.gt_response_ids[item], self.gt_belief_length[item], torch.tensor(self.mc_labels[item])
...
def decode_belief_response(args, generated_sequence, tokenizer):
    out = generated_sequence.tolist()
    # decode_sequence = []
    text = tokenizer.decode(out[0], clean_up_tokenization_spaces=True)
    text = text[: text.find(args.stop_token) if args.stop_token else None]
    # decode_sequence.append(text) #  decode_sequence = [' XXXXX']
    
    return text

def evaluatesession2(args, source):

    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.rewardmodel_type]
    ...
    # source is str  user : i'm looking for a restaurant that offers british food in a moderate price range, can you help me with that?=>belief : restaurant pricerange = moderate ; area = sf ;system : yes i have many in the center of town and on in the [value_area]. do you have a preference for area?
    if args.rewardmodel_type == 'bert':
        dial_history = tokenizer.cls_token + source.split("=>")[0] + tokenizer.sep_token
    else:
        dial_history = tokenizer.cls_token + source.split("=>")[0] + tokenizer.sep_token + tokenizer.sep_token

    belief_response = source.split("=>")[-1] + ' ' + tokenizer.sep_token
    # context = tokenizer.cls_token + source + ' ' + tokenizer.sep_token
    context = dial_history + belief_response
    print("context: \n", context)
    source = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(context))
    belief_response_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(belief_response))
    token_id = [0] * (len(source) - len(belief_response_id)) + [1] * len(belief_response_id)
    ...
    if len(source) < max_seq:
        ...
        source += [tokenizer.pad_token_id] * (max_seq - len(source))
    ...
    inputs = torch.tensor([source]).to(args.device)
    masks = torch.tensor([attention_mask]).to(args.device)
    tokens = torch.tensor([token_id]).to(args.device)
    model.eval()
    with torch.no_grad():
        if args.rewardmodel_type == 'bert':
            outputs = model(input_ids=inputs, attention_mask=masks, token_type_ids=tokens)
        else:
            outputs = model(input_ids=inputs, attention_mask=masks, token_type_ids=None)
        # print(outputs)
...
def evaluatesession(args, source, token_id):
    ...
    #belief_response = source.split("=>")[-1]
    source = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(source))
    #belief_response_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(belief_response))
    token_id = token_id + system_token_id *(len(source) - len(token_id))
    inputs = torch.tensor([source]).to(args.device)
    tokens = torch.tensor([token_id], dtype=torch.long, device=args.device)
    model.eval()
    with torch.no_grad():
        outputs = model(input_ids=inputs, token_type_ids=tokens)
        # print("outputs: \n", len(outputs))
...
def sample_batch_sequence(model, tokenizer, gt_belieflen, input_ids, token_type_ids, system_token_id, gtresponse_id, temperature=1, top_k=0, top_p=0.0, device='cpu'):
    # model.train()
    input_ids = torch.tensor(input_ids, dtype=torch.long, device=device)
    input_token_type_ids = torch.tensor(token_type_ids, dtype=torch.long, device=device)
    system_token_id_forsample = torch.tensor(system_token_id, dtype=torch.long, device=device).unsqueeze(0)
    gt_response_len = len(gtresponse_id)
    generated = input_ids
    context = input_ids
    log_probs = []
    ...
    model.eval()
    with torch.no_grad():
        for i in range(gt_belieflen):
            # i = torch.tensor(i, dtype=torch.int, device=device)
            inputs = {'input_ids': context, 'past': past, 'token_type_ids':input_token_type_ids}
            ...
            if next_token == system_token_id:
                break
            log_probs.append(log_prob)
            context = next_token
            generated = torch.cat((generated, next_token), dim=1)
            input_token_type_ids = system_token_id_forsample
            decode_belief_len = len(log_probs)

    model.train()
    past = None # using past

    context = generated
    token_type_ids = token_type_ids + decode_belief_len * system_token_id
    token_type_ids = torch.tensor(token_type_ids, dtype=torch.long, device=device)
    gtresponse_id = torch.tensor(gtresponse_id, dtype=torch.long, device=device)
    log_probs = []
    for j in range(gt_response_len):
        j = torch.tensor(j, dtype=torch.int, device=device)
        ...
        # next_token = multi_dist.sample()
        # print("next token: \n", next_token)
        next_token = torch.tensor([gtresponse_id[j]], dtype=torch.long, device=device)
        # print("next token in gt", next_token)
        log_prob = multi_dist.log_prob(next_token)
        next_token = next_token.unsqueeze(0) 
        log_probs.append(log_prob)
        ...
        context = next_token
        generated = torch.cat((generated, next_token), dim=1)
        token_type_ids = system_token_id_forsample
    ...
    # past = None # using past
    decode_len = decode_len + decode_belief_len
    
    return log_probs, generated, decode_len
...
def train(args, train_dataset, model, tokenizer):
    ...
    for e in train_iterator:
        for step, batch in enumerate(train_dataloader):
            ...
            input_id, token_type_id, gtresponse_id, gt_belieflen, mc_label = batch

            # model.train()
            RL_log_prob, generated, decode_len = sample_batch_sequence(
                model=model,
                tokenizer=tokenizer,
                input_ids=input_id,
                token_type_ids=token_type_id,
                system_token_id=system_token_id,
                gt_belieflen=gt_belieflen,
            ...
            # full_sequence_len = len(token_type_id) + decode_len
            # print("full sequence length: \n", full_sequence_len)
            # assert full_sequence_len == len(generated)
            if args.rewardmodel_type == 'gpt2':
                generated = decode_belief_response(args, generated, tokenizer)
                pred, reward = evaluatesession(args, generated, token_type_id)
            else:
                generated = decode_belief_response(args, generated, tokenizer)
                # print("generated: \n", generated) #str
                # user : i'm looking for a restaurant that offers british food in a moderate price range, can you help me with that?=>belief : restaurant pricerange = moderate ; area = sf ;system : yes i have many in the center of town and on in the [value_area]. do you have a preference for area?
                pred, reward = evaluatesession2(args, generated)
            print("true mc_label: \n", mc_label)
...
def main():
    ...
    parser.add_argument("--replace_systurn", action='store_true')
    parser.add_argument("--positive_reward", default=0.5, type=float)
    parser.add_argument("--negative_reward", default=-0.001, type=float)
    parser.add_argument("--repeat_token", action='store_true')
