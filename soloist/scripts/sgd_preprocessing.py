...
print(sys.argv)
for split in ['test']:
    schema_info = json.load(open(f'./data/dstc8-schema-guided-dialogue/{split}/schema.json'))
    schema_info = dict([(i['service_name'],i) for i in schema_info])
    for file in glob.glob(f'./data/dstc8-schema-guided-dialogue/{split}/dialogues_*.json'):
...
json.dump(examples, open('sgd.test.json','w'), indent=2)