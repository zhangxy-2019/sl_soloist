...
class JsonDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', max_seq=80, max_turn=1, seperator=' & '):
        ...
        for id, example in enumerate(examples):
            ...
            source = [tokenizer.cls_token_id] + context_ids + [tokenizer.sep_token_id]*2 + belief_id + response_id
            # print("source: \n", source)
            ...
            if args.add_same_belief_response_prediction:
                for _ in range(args.num_candidates):
                    ...
                    source = [tokenizer.cls_token_id] + context_ids + [tokenizer.sep_token_id]*2 + new_belief_id + new_response_id
            ...
            if args.add_response_prediction:
                for _ in range(args.num_candidates):
                    ...
                    source = [tokenizer.cls_token_id] + context_ids + [tokenizer.sep_token_id]*2 + belief_id + new_response_id
            ...
            if args.add_belief_prediction:
                for _ in range(args.num_candidates):
                    ...
                    source = [tokenizer.cls_token_id] + context_ids + [tokenizer.sep_token_id]*2 + new_belief_id + response_id
...
def f1_score_func(preds, labels, file_path):
    examples = json.load(open(file_path))
    preds_flat = np.argmax(preds, axis=1).flatten()
    labels_flat = labels.flatten()
    error_analysis = []

    label_dict = {"false": 1, "true": 0}
    label_dict_inverse = {v: k for k, v in label_dict.items()}
    for i in range(len(labels_flat)):
        if preds_flat[i] != labels_flat[i]:
            error_dict = {}
            error_dict["pred_label"] = label_dict_inverse[preds_flat[i]]
            
            error_dict["true_label"] = label_dict_inverse[labels_flat[i]]
            error_analysis.append(error_dict)
    
    return f1_score(labels_flat, preds_flat, average='weighted'), error_analysis
...
def accuracy_per_class(preds, labels):
    ...
    for label in np.unique(labels_flat):
        ...
        result_dict[f'Class {label_dict_inverse[label]} Accuracy:'] = len(y_preds[y_preds==label])/len(y_true)
...
def evaluate(args, model, checkpoint, eval_dataset, eval_dataloader, tokenizer, prefix=""):
    ...
    # print("true label ids: \n", out_label_ids)
    val_f1, error_analysis = f1_score_func(preds, out_label_ids, file_path=args.eval_data_file)
    out_error_analyfile = os.path.join(eval_output_dir, "error_analysis_result.json")
    json.dump(error_analysis, open(out_error_analyfile, 'w'), indent=2)

    result = {"F1 Score (Weighted)": val_f1}
    class_dict = accuracy_per_class(preds, out_label_ids)
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        if args.eval_all_checkpoints:
            ...
            checkpoints.sort(key=lambda x:int(x.split('-')[-1] if x.find('checkpoint') != -1 else "0"), reverse=True)
            logging.getLogger("transformers.modeling_utils").setLevel(logging.WARN)  # Reduce logging
        logger.info("Evaluate the following checkpoints: %s", checkpoints)
        total_eval_output_file = os.path.join(args.output_dir, "total_eval_results.txt")
        with open(total_eval_output_file, "w") as writer:
            ...
            for checkpoint in checkpoints:
                ...
                model.to(args.device)
                result = evaluate(args, model, checkpoint, eval_dataset, eval_dataloader, tokenizer, prefix=prefix)
