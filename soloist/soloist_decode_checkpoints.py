...
MAX_LENGTH = int(110)  # Hardcoded max length to avoid infinite loop
...
def sample_sequence(model, tokenizer, length, context, token_type_ids, system_token_id, num_samples=1, temperature=1, top_k=0, top_p=0.0, repetition_penalty=1.0, device='cpu'):
    ...
    end_token_id = torch.tensor(tokenizer.eos_token_id, dtype=torch.long, device=device).unsqueeze(0).repeat(num_samples, 1)
    user_token_id = tokenizer.convert_tokens_to_ids(tokenizer.tokenize("user"))
    user_token_id = torch.tensor(user_token_id, dtype=torch.long, device=device).unsqueeze(0).repeat(num_samples, 1)
    ...
    with torch.no_grad():
        for _ in range(length):
            ...
            if next_token == user_token_id:
                break
            if next_token == end_token_id:
                break
    return generated

def decode(tokenizer, model, args, prefix, inputs, checkpoint_dir, system_token_id, user_token_id):
    """ decode saved checkpoints and generate decoded text files """

    logger.info("***** Running decoding {} *****".format(prefix))
    ...
    for idx in range(len(inputs)):
        ...
        print(output_tests)
    output_test_file  = os.path.join(checkpoint_dir, "test_decode.json")
    json.dump(output_tests, open(output_test_file,'w'), indent=2)
    return text
...
def main():
    ...
    checkpoints.sort(key=lambda x:int(x.split('-')[-1] if x.find('checkpoint') != -1 else "0"))
    logging.getLogger("transformers.modeling_utils").setLevel(logging.WARN)  # Reduce logging
    logger.info("Decode the following checkpoints: %s", checkpoints)
    for checkpoint in checkpoints:
        ...
        system_token_id = tokenizer.convert_tokens_to_ids(['system'])
        user_token_id = tokenizer.convert_tokens_to_ids(['user'])
        text = decode(tokenizer=tokenizer, model=model, args=args, prefix=prefix, inputs=inputs, checkpoint_dir=checkpoint, system_token_id=system_token_id, user_token_id=user_token_id)
