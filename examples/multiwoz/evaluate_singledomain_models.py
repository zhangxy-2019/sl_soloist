...
class BLEUScorer(object):
    ...
    def score(self, hypothesis, corpus, n=1):
        ...
        for hyps, refs in zip(hypothesis, corpus):
            ...
            # print("refs: \n", refs)
            hyps = [hyp.split() for hyp in hyps]
            # print("hyps after split: \n", hyps)
            refs = [ref.split() for ref in refs]
            # print("refs after split: \n", refs)
            # import pdb
            # pdb.set_trace()
            # hyps = [hyps]
            # Shawn's evaluation
...
class MultiWozEvaluator(BaseEvaluator):
    def __init__(self, data_name, checkpoint):
        self.data_name = data_name
        self.slot_dict = delex.prepareSlotValuesIndependent()
        self.delex_dialogues = json.load(file('data/multi-woz/single_domain_testdata.delex.json'))
    ...
    def _evaluateGeneratedDialogue(self, dialog, goal, realDialogue, real_requestables, soft_acc=False):
        ...
        For the Success rate we look for requestable slots"""
    ...
    def evaluateModel(self, dialogues, dialogues_bs, real_dialogues=False, mode='valid'):
        ...
        if real_dialogues:
            ...
            for dialogue in dialogues.keys():
                ...
                sys_turns = data['log'] # if idx % 2 == 0, then user turn; else system turn
                for idx, turn in enumerate(sys_turns):
                    if idx % 2 == 1:
                        corpus_turns.append([turn['text']]) # system turn
                ...
                if len(model_turns) == len(corpus_turns):
                    ...
                    print("model_turns\n", model_turns)
                    print("corpus_turns\n", corpus_turns)
                    print(dialogue.split('.')[0].upper()+'.json')
        ...
        report = "***** %s *****: \n" % str(checkpoint)
        report += 'Corpus Matches : %s' % str(matches / float(total)) + "\n"
        report += 'Corpus Success : %s'% str(successes / float(total)) + "\n"
        report += ' Corpus BLEU : %s' % str(blue_score) + "\n"
        report += 'Combined Score : %s'% str(combined) + "\n"
        report += 'Total number of dialogues: %s ' % total

        # combined = (successes/float(total) + matches/float(total))/2 + blue_score
        # print(f'Combined Score {combined}')
        output_eval_file = os.path.join(self.checkpoint, "test_eval_results.txt")
        with open(output_eval_file, "w") as writer:
            writer.write("***** Eval results *****\n")
            writer.write("Corpus Matches = %s\n" % str(matches / float(total)))
            writer.write("Corpus Success = %s\n" % str(successes / float(total)))
            writer.write("Corpus BLEU = %s\n" % str(blue_score))
            writer.write("Total number of dialogues: %s\n" % str(total))
            writer.write("Combined Score  = %s\n" % str(combined))
            
        print(report)
        combined = (successes/float(total) + matches/float(total))/2 + blue_score
        print(f'Combined Score {combined}')
        # corpus_match = matches / float(total)
        # corpus_success = successes / float(total)
        # result = {
        #     "Corpus Matches": corpus_match,
        #     "Corpus Success": corpus_success,
        #     "Corpus BLEU": blue_score,
        #     "Total number of dialogues": total,
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--eval_output_dir", type=str, default="", help="Prediction file will be written to this path.")
    
    args = parser.parse_args()
    mode = args.eval_mode
    # results = {}
    checkpoints = [args.model_name_or_path]
    checkpoints = list(os.path.dirname(c) for c in sorted(glob.glob(args.model_name_or_path + '/**/', recursive=True)))
    print(checkpoints)
    checkpoints.sort(key=lambda x:int(x.split('-')[-1] if x.find('checkpoint') != -1 else "100000000"))

    # logging.getLogger("transformers.modeling_utils").setLevel(logging.WARN)  # Reduce logging
    # logger.info("Evaluate the following checkpoints: %s", checkpoints)
    total_eval_output_file = os.path.join(args.model_name_or_path, "total_eval_results.txt")
    with open(total_eval_output_file, "w") as writer:
        writer.write("***** Total Eval results *****\n")
        for checkpoint in checkpoints: 
            print(checkpoint)
            evaluator = MultiWozEvaluator(mode, checkpoint)
            ...
            # model.eval()
            f = os.path.join(checkpoint, "test_decode.json")
            res, res_bs = parse_decoding_results(f, mode)
            human_proc_data = res 
            generated_data = human_proc_data # generated response
            generated_proc_belief = res_bs # generated belief state
            evaluator.evaluteDST(generated_data, generated_proc_belief, True, mode=mode)
            report, _, _, = evaluator.evaluateModel(generated_data, generated_proc_belief, True, mode=mode)
            writer.write("%s\n" % (report))
    # total_eval_output_file = os.path.join(args.model_name_or_path, "total_eval_results.txt")
    # with open(total_eval_output_file, "w") as writer:
    #     writer.write("***** Total Eval results *****\n")
